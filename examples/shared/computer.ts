/**
 * Shared computer tool module for portabledesktop examples.
 *
 * Wraps the Go CLI binary behind a DesktopComputer class that owns
 * coordinate scaling, viewport tracking, screenshot downscaling, PNG
 * dimension parsing, modifier-key handling, and abort-signal plumbing.
 *
 * Both the agent and demo examples import from here so the logic
 * stays in one place.
 */
...
export interface DesktopComputerOptions {
  /** Path to the portabledesktop CLI binary. */
  bin: string;
  /** Native display width in pixels. */
  displayWidthPx: number;
  /** Native display height in pixels. */
  displayHeightPx: number;
  /** Timeout for screenshot commands in milliseconds. */
...
export interface AnthropicComputer20250124ToolOptions extends AnthropicComputerToolSharedOptions {}

export interface AnthropicComputer20251124ToolOptions extends AnthropicComputerToolSharedOptions {
  enableZoom?: boolean;
}

export interface OpenAIComputerToolOptions extends DesktopComputerOptions {
  description?: string;
  enableZoom?: boolean;
}
...
// ---------------------------------------------------------------------------
// Screenshot sizing and PNG helpers
// ---------------------------------------------------------------------------
...
export function computeScaledScreenshotSize(
  width: number,
  height: number,
  ...
  const totalPixelsScale = Math.sqrt(
    ANTHROPIC_MAX_SCREENSHOT_TOTAL_PIXELS / totalPixels,
  );
...
// ---------------------------------------------------------------------------
// toModelOutput — canonical return format for AI SDK tools
// ---------------------------------------------------------------------------

function toModelOutput(output: ComputerToolOutput): {
  type: "content";
  value: Array<
    | { type: "image-data"; data: string; mediaType: "image/png" }
...
// ---------------------------------------------------------------------------
// DesktopComputer — wraps the CLI with coordinate transforms
// ---------------------------------------------------------------------------
...
export class DesktopComputer {
  ...
  constructor(options: DesktopComputerOptions) {
    ...
    this.displayWidthPx = requirePositiveInteger(
      options.displayWidthPx,
      "displayWidthPx",
    );
    this.displayHeightPx = requirePositiveInteger(
      options.displayHeightPx,
      "displayHeightPx",
    );
  ...
  private execVoid(args: string[]): void {
    execFileSync(this.bin, args, { timeout: 30_000 });
  }
  ...
  private clampPoint(
    ...
    const x = Math.max(
      0,
      Math.min(this.displayWidthPx - 1, Math.round(rawX)),
    );
    const y = Math.max(
      0,
      Math.min(this.displayHeightPx - 1, Math.round(rawY)),
    );
    return [x, y];
  }
  ...
  private clampRegion(
    ...
    const safe = region.map((value) =>
      Number.isFinite(value) ? value : 0,
    ) as [number, number, number, number];
    const [x1, y1, x2, y2] = safe;
    const left = Math.max(
      0,
      Math.min(this.displayWidthPx - 1, Math.floor(Math.min(x1, x2))),
    );
    const top = Math.max(
      0,
      Math.min(this.displayHeightPx - 1, Math.floor(Math.min(y1, y2))),
    );
    const right = Math.max(
      left + 1,
      Math.min(this.displayWidthPx, Math.ceil(Math.max(x1, x2))),
    );
    const bottom = Math.max(
      top + 1,
      Math.min(this.displayHeightPx, Math.ceil(Math.max(y1, y2))),
    );
    return [left, top, right, bottom];
  }

  private scaledToNativeAxis(
    value: number,
    scaledSpan: number,
    nativeSpan: number,
  ): number {
  ...
  private unscalePoint(
    ...
    const localX = this.scaledToNativeAxis(
      point[0],
      viewport.scaledWidth,
      viewport.nativeWidth,
    );
    const localY = this.scaledToNativeAxis(
      point[1],
  ...
  private scalePoint(
    ...
    const x = this.nativeToScaledAxis(
      localX,
      viewport.nativeWidth,
      viewport.scaledWidth,
    );
    const y = this.nativeToScaledAxis(
      localY,
      viewport.nativeHeight,
      viewport.scaledHeight,
    );
  ...
  private unscaleRegion(
    ...
    const [startX, startY] = this.clampPoint(
      this.unscalePoint([region[0], region[1]]),
    );
    const [endX, endY] = this.clampPoint(
      this.unscalePoint([region[2], region[3]]),
    );
    const left = Math.max(0, Math.min(startX, endX));
    const top = Math.max(0, Math.min(startY, endY));
    const right = Math.max(
      left + 1,
      Math.min(this.displayWidthPx, Math.max(startX, endX) + 1),
    );
    const bottom = Math.max(
      top + 1,
      Math.min(this.displayHeightPx, Math.max(startY, endY) + 1),
    );
    return [left, top, right, bottom];
  }
  ...
  private async withModifierKeys<T>(
    ...
    try {
      for (const key of keys) {
        throwIfAborted(signal);
        this.execVoid(["keyboard", "down", key]);
      ...
      for (const key of [...pressedKeys].reverse()) {
        try {
          // Ensure modifiers are released even when execution is
          // aborted.
          this.execVoid(["keyboard", "up", key]);
        } catch {
  ...
  private buildCapturePlan(
    ...
    if (!region) {
      const scaled = computeScaledScreenshotSize(
        this.displayWidthPx,
  ...
  private capturePng(
    ...
    const tw = capturePlan.viewport.scaledWidth;
    const th = capturePlan.viewport.scaledHeight;
    ...
    if (capturePlan.region) {
      const [left, top, right, bottom] = capturePlan.region;
      args.push(
        "--x",
        String(left),
        "--y",
    ...
    if (this.screenshotTimeoutMs > 0) {
      args.push("--timeout-ms", String(this.screenshotTimeoutMs));
    }
    ...
    const actualDimensions = parsePngDimensions(data);
    this.latestViewport = {
      ...capturePlan.viewport,
      scaledWidth:
        actualDimensions?.width ?? capturePlan.viewport.scaledWidth,
      scaledHeight:
        actualDimensions?.height ?? capturePlan.viewport.scaledHeight,
    };
  ...
  private currentCursor(signal?: AbortSignal): [number, number] {
    throwIfAborted(signal);
    const result = this.exec(["cursor", "--json"]);
    const pos = JSON.parse(result.trim());
    const nativePoint = this.clampPoint([pos.x, pos.y]);
    const modelPoint = this.scalePoint(nativePoint);
    return this.clampPoint(modelPoint);
  }

  // -----------------------------------------------------------------------
  // Repeated click helper
  // -----------------------------------------------------------------------
  ...
  private repeatClick(
    ...
    for (let i = 0; i < repeatCount; i += 1) {
      throwIfAborted(signal);
      this.execVoid(["mouse", "click", button]);
    }
  }
  ...
  async execute(
    ...
    switch (input.action) {
      case "key": {
        ...
        this.execVoid(["keyboard", "key", text]);
      ...
      case "hold_key": {
        ...
        const durationMs = Math.max(
          10,
          Math.round((input.duration ?? 0.25) * 1000),
        );
        try {
        ...
        return {
          kind: "text",
          text: `held keys for ${durationMs}ms: ${keys.join("+")}`,
        };
      }
      case "type": {
        if (!input.text) {
      ...
      case "left_click": {
        ...
        await this.withModifierKeys(input.text, signal, async () => {
          this.execVoid(["mouse", "click", "left"]);
        });
        return { kind: "text", text: "left click" };
      }
      case "left_click_drag": {
        const [startX, startY] = this.requirePoint(
          input.start_coordinate,
          "start_coordinate",
        );
        const [endX, endY] = this.requirePoint(
          input.coordinate,
          "coordinate",
        );
        ...
        try {
          ...
          try {
            // Ensure button state is restored when a drag is aborted
            // mid-flight.
            this.execVoid(["mouse", "up", "left"]);
          } catch {
      ...
      case "right_click": {
        ...
        await this.withModifierKeys(input.text, signal, async () => {
          this.execVoid(["mouse", "click", "right"]);
        });
      ...
      case "middle_click": {
        ...
        await this.withModifierKeys(input.text, signal, async () => {
          this.execVoid(["mouse", "click", "middle"]);
        });
      ...
      case "double_click": {
        ...
        await this.withModifierKeys(input.text, signal, async () => {
          this.repeatClick("left", 2, signal);
        });
      ...
      case "triple_click": {
        ...
        await this.withModifierKeys(input.text, signal, async () => {
          this.repeatClick("left", 3, signal);
        });
      ...
      default: {
        throw new Error(
          `unsupported action: ${String((input as { action?: string }).action ?? "unknown")}`,
        );
      }
    }
  }
...
// ---------------------------------------------------------------------------
// Factory: Anthropic computer_20250124 tool
// ---------------------------------------------------------------------------

export function createAnthropicComputer20250124Tool(
  options: AnthropicComputer20250124ToolOptions,
) {
  ...
  return anthropicProvider.tools.computer_20250124<ComputerToolOutput>({
    ...
    execute: async (
      input,
      executeOptions: AnthropicComputer20250124ExecuteOptions,
...
// ---------------------------------------------------------------------------
// Factory: Anthropic computer_20251124 tool
// ---------------------------------------------------------------------------

export function createAnthropicComputer20251124Tool(
  options: AnthropicComputer20251124ToolOptions,
) {
  const computer = new DesktopComputer(options);
  return anthropicProvider.tools.computer_20251124<ComputerToolOutput>({
    ...
    execute: async (
      input,
      executeOptions: AnthropicComputer20251124ExecuteOptions,
    ) => computer.execute(input, { abortSignal: executeOptions.abortSignal }),
    toModelOutput: ({ output }) => toModelOutput(output),
  });
}
...
// ---------------------------------------------------------------------------
// Factory: OpenAI computer tool
// ---------------------------------------------------------------------------

export function createOpenAIComputerTool(options: OpenAIComputerToolOptions) {
  const computer = new DesktopComputer(options);
