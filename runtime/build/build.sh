...
# trimmed xkeyboard-config) for the portabledesktop Go runtime module, and
...
#
# The build command can be replaced through PORTABLEDESKTOP_RUNTIME_BUILDX, which
# defaults to "docker buildx build". CI may set it to a Depot invocation, which
...
build_cmd_str="${PORTABLEDESKTOP_RUNTIME_BUILDX:-docker buildx build}"
read -r -a build_cmd <<<"$build_cmd_str"
if [[ "${#build_cmd[@]}" == 0 ]]; then
	error "PORTABLEDESKTOP_RUNTIME_BUILDX must not be empty"
fi
is_default_buildx=0
...
if [[ -n "$builder" ]]; then
	...
	if ! docker buildx inspect portabledesktop-runtime >/dev/null 2>&1; then
		logrun docker buildx create --name portabledesktop-runtime --driver docker-container >/dev/null
	fi
	builder_args+=(--builder portabledesktop-runtime)
fi
